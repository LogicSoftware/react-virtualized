...
test("save position after resize and then scroll in container", async () => {
    ...
    expect(resizeFn.mock.calls).toEqual([[{ width: 500, height: 700 }], [{ width: 400, height: 600 }]]);
});
...
test("react on container resize without window changing", async () => {
    ...
    await page.evaluate(() => {
        ...
        render(
            createElement(WindowScroller, { scrollElement: container, onResize: window.resizeFn }, () => null),
            container,
        );
    });
    ...
    expect(resizeFn.mock.calls).toEqual([[{ width: 1000, height: 800 }], [{ width: 500, height: 700 }]]);
});
