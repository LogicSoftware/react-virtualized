...
const headerRenderer = ({ dataKey, label }) => {
    ...
    return (
        <>
            ...
            {showSortIndicator && <SortIndicator sortDirection={sortState.sortDirection[dataKey]} />}
        </>
    );
};

// Connect sortState to Table by way of the 'sort' prop:
<Table {...tableProps} sort={sortState.sort} sortBy={undefined} sortDirection={undefined}>
